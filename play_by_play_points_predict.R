...
data <- read.csv(file = 'data\\perplaydata.csv', sep=',', header=T)

data$Visitor.Team <- trim(data$Visitor.Team)
data$Home.Team <- trim(data$Home.Team)

FBS.Team <- read.csv(file = 'data\\records.csv', sep=',', header=T)
...
{
  maxSoS <- mean(working$SOS)
  return(sqrt(awaySoS / maxSoS ))
}
Defensive.SoS.Adjust <- function(awaySoS)
{
  maxSoS <- mean(working$SOS)
  return(sqrt(maxSoS / awaySoS ))
}
...
setup.predict <- function(teamA, teamB, runCount)
{
  ...
  home.yards.per.pass <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamA,]$Yards.Per.Pass, 
                                sd=stat.Std.Dev[stat.Std.Dev$TEAM == teamA,]$Yards.Per.Pass)
  home.yards.per.carry <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamA,]$Yards.Per.Carry, 
                                 sd=stat.Std.Dev[stat.Std.Dev$TEAM == teamA,]$Yards.Per.Carry)
  home.points.per.play <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamA,]$Points.Per.Play, 
  ...
  home.yards.per.pass.allowed <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamA,]$Opp.Yards.Per.Pass, 
                                        sd=stat.Std.Dev[stat.Std.Dev$TEAM == teamA,]$Opp.Yards.Per.Pass)
  home.yards.per.carry.allowed <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamA,]$Opp.Yards.Per.Carry, 
                                         sd=stat.Std.Dev[stat.Std.Dev$TEAM == teamA,]$Opp.Yards.Per.Carry)
  home.points.per.play.allowed <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamA,]$Opp.Points.Per.Play, 
  ...
  home.percent.pass <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamA,]$Pass.Percent, 
  ...
  away.yards.per.pass <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamB,]$Yards.Per.Pass, 
                                sd=stat.Std.Dev[stat.Std.Dev$TEAM == teamB,]$Yards.Per.Pass)
  away.yards.per.carry <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamB,]$Yards.Per.Carry, 
                                 sd=stat.Std.Dev[stat.Std.Dev$TEAM == teamB,]$Yards.Per.Carry)
  away.points.per.play <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamB,]$Points.Per.Play, 
  ...
  away.yards.per.pass.allowed <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamB,]$Opp.Yards.Per.Pass, 
                                        sd=stat.Std.Dev[stat.Std.Dev$TEAM == teamB,]$Opp.Yards.Per.Pass)
  away.yards.per.carry.allowed <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamB,]$Opp.Yards.Per.Carry, 
                                         sd=stat.Std.Dev[stat.Std.Dev$TEAM == teamB,]$Opp.Yards.Per.Carry)
  away.points.per.play.allowed <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamB,]$Opp.Points.Per.Play, 
  ...
  away.percent.pass <-  rnorm(n=runCount, mean=stat.Averages[stat.Averages$TEAM == teamB,]$Pass.Percent, 
...
neutral.game <- function(row)
{
  ...
  game[["Home Score"]] <- home.SoS.adjust*home.points
  game[["Away Score"]] <- away.SoS.adjust*away.points
  return(game)
}
...
game <- function(row)
{
  ...
  game[["Home Score"]] <- 3+home.SoS.adjust*home.points
  game[["Away Score"]] <- away.SoS.adjust*away.points
  return(game)
}
